...
# Regenerate documentation screenshots (uses Textual pilot with demo data)
python scripts/take_screenshots.py
```

There are no tests, linters, or formatters configured for this project.

Note: `run.sh` exists but references a stale `venv/` path — use `./run.py` instead.
...
- Tab 3 (SearchView): Registry browser with real-time filtering and filter bar (period, params size, cloud)
...
- Web scraper: Fetches model catalog from `ollama.com/library` and tags from `/library/{model}/tags` using `urllib.request` + regex HTML parsing. Extracts: name, sizes, description, updated (relative + exact date), cloud badge, capabilities, pull count
...
**Async generator for pull**: `OllamaClient.pull_model()` is an async generator (uses `yield`), not a regular async method. It streams progress lines that `PullProgressScreen` consumes via `async for`. The progress screen parses percentage values from the text and strips ANSI escape codes.
...
2. Registry data is scraped via HTML regex parsing — fragile, depends on ollama.com HTML structure (CSS class names like `x-test-size`, `text-neutral-800`, `col-span-2 text-neutral-500`, `text-cyan-500` for cloud badge, `x-test-capability`, `x-test-pull-count`)
...
- Recommended: `pipx install git+https://github.com/elmisi/ollama-cli-tui.git`
- `install.sh` is local-clone-only (`./install.sh` from repo). Creates a venv at `.venv/` and a wrapper in `~/.local/bin/ollama-tui`.
- `pyproject.toml` defines a `[project.scripts]` entry point `ollama-tui` for pip-based install via hatchling.

## Keyboard Bindings

Global: `1`/`2`/`3` switch tabs, `left`/`right` cycle tabs, `q` quit.
Models tab: `r` refresh, `d` delete, `Enter` show info.
Running tab: `r` refresh, `s` stop model.
Search tab: `r` refresh, `p` pull, `/` focus search input, `Escape` back to table.

## Logging

Debug logs written to `ollama_tui.log` in project root. The path is computed as 4x `Path.parent` from `app.py` — if the source tree depth changes, this breaks.
